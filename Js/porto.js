...
window.onload = async () => {
  ...
  fetch(`${API_URL}/api/instellingen-systeem`).then(r=>r.json()).then(d=>{ 
  if(d.ping_interval) {
    window._pingInterval = parseInt(d.ping_interval);
    console.log('PING INTERVAL - Loaded from settings:', window._pingInterval, 'seconds');
  } else {
    window._pingInterval = 30;
    console.log('PING INTERVAL - Using default 30 seconds');
  }
}).catch(err => {
  window._pingInterval = 30;
  console.log('PING INTERVAL - Error loading settings, using default 30 seconds:', err);
});
  const u = getUser();
...
if (ovdView) {
  ovdView.classList.remove('hidden');
    ...
    if (!heeftOpco && !isAdmin) { 
      const b = document.getElementById('btn-opco'); 
      if (b) b.style.display = 'none'; 
    }
    if (!heeftOvd && !isAdmin)  { 
      const b = document.getElementById('btn-ovd');  
      if (b) b.style.display = 'none'; 
    }
    if (!heeftOc && !isAdmin)   { 
      const b = document.getElementById('btn-oc');   
      if (b) b.style.display = 'none'; 
    }
    if (!heeftOps && !isAdmin)  { 
      const b = document.getElementById('btn-ops');  
      if (b) b.style.display = 'none'; 
    }
      ...
      if (wachtrij.length > 0) {
        ...
        if (!window._pingHerhaalTimer) {
          ...
          window._pingHerhaalTimer = setInterval(() => {
            ...
            console.log('PING INTERVAL - Wachtrij interval ping triggered');
...
function toonOntkoppelModalMetDatabaseData(unit, partnerData) {
  console.log('ONTKOPPELEN - Toon modal met database data:', partnerData);
